...
export async function buildApp({ provider, origin, sessionKey, secure = true, audit = () => {}, secrets = [], headlineGenerator = suggestHeadlines, voiceInterpreter = interpretVoiceCommand, webRoot = fileURLToPath(new URL('../dist', import.meta.url)) }) {
  ...
  app.get('/api/v1/dashboard', { schema: { headers: roomHeaders } }, (req, reply) => rpc(req, reply, 'tn_painel_operacao', { redacao_id: req.headers['x-redacao-id'] }));
  app.get('/api/v1/agentes', { schema: { headers: roomHeaders } }, (req, reply) => rpc(req, reply, 'tn_painel_agentes', { redacao_id: req.headers['x-redacao-id'] }));
